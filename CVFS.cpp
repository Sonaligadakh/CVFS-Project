...
void man(char *name)
{
	...
	else if(strcmp(name,"read")==0)
	{
		...
		printf("Usage:reaf File_Name No_Of_Bytes_To_Read\n ");
	}
	...
	{
		printf("Description:Used yo list all information of files\n ");
		printf("Usage: ls\n");
	}
	...
	{
		printf("Description:Used to open existinf File\n ");
...
int WriteFile(int fd,char *arr,int isize)
{
	...
	(UFDTArr[fd].qptrfiletable->ptrinode->FileActualSize)=(UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)+isize;
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode==READ)||(UFDTArr[fd].ptrfiletable->mode==READ+WRITE))
	{
		if(from==CURRENT)
		{
			...
			if(((UFDTArr[fd].ptrfiletable->readoffset)=(UFDTArr[fd].ptrfiletable->readoffset)+size)<0)			return -1;
	...
	while(1)
	{
		...
		else if(count==2)
		{
			...
			else if(strcmp(command[0],"truncate")==0)
			{
				...
				if(ret==-1)
					printf("ERROR:Incorect paramerter\n");
			}
			else
			{
				printf("\nERROR: Command not found!!!\n");
		...
		else if(count==3)
		{
			...
			else if(strcmp(command[0],"open")==0)
			{
				...
				if(ret==-1)
					printf("ERROR:Incorrect parameeters\n");
				if(ret==-2)
